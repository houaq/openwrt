...
//end modified by zhangkaibo add PCIe auto detect feature on x55/x24 platform. 20200605
    ...
    while ((ent = readdir(pDir)) != NULL) {
        ...
        snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.4/net", dir,
             ent->d_name);
        psubDir = opendir(pl->subdir);
        if (psubDir == NULL) {
        dbg_time("Cannot open directory: %s, errno: %d (%s)",
        ...
        snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.4/GobiQMI", dir,
             ent->d_name);
        if (access(pl->subdir, R_OK)) {
        snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.4/usbmisc",
             dir, ent->d_name);
        if (access(pl->subdir, R_OK)) {
            snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.4/usb",
                 dir, ent->d_name);
            if (access(pl->subdir, R_OK)) {
            dbg_time("no GobiQMI/usbmic/usb found in %s/%s:1.4",
                 dir, ent->d_name);
            continue;
            }
        }
...
//begin modified by zhangkaibo limit input parameters. mantis 0048965  20200605
        {
            ...
            if(profile.auth < 0 || profile.auth > 2)
            {
                dbg_time("auth must be 0~2");
                return -1;
            }
        }
//end modified by zhangkaibo limit input parameters. mantis 0048965  20200605
